...
country_name_map <- c("USA", "Grossbritannien", "Deutschland", "Frankreich",
...
plot_trend_lines <- function(df, trendy_ctries) {
  ...
  gg <- df_here %>% 
    ...
    ggplot(aes(x = CallDecisionYear, y = pctg, colour = Country,
           tooltip = paste0("Land: ", Country, "<br>",
                            "Anteil: ", paste0(round(pctg * 100, 1), " %"), "<br>",
                            "Anzahl: ", n, "<br>",
                            "Jahr: ", CallDecisionYear, "<br>"),
...
gg_europe <- df_europe %>% 
  ggplot(aes(x = CallDecisionYear, y = pctg, colour = Continent,
             tooltip = paste0("Kontinent: ", Continent, "<br>",
                              "Anteil: ", paste0(round(pctg * 100, 1), " %"), "<br>",
                              "Anzahl: ", n, "<br>",
                              "Jahr: ", CallDecisionYear, "<br>"),
...
<div class="plot-box">
<div class="plot-title">Andere vielbesuchte Länder</div>
```{r trends2, out.width="100%", fig.height=6}
ggiraph_trendy_rest
```
<div class="caption">
Jährlicher Anteil der Postdoc.Mobility-Aufenthalte in ausgewählten Ländern  
Für die nicht aufgeführten Länder waren keine auffälligen Trends festzustellen.
</div>
</div>

Die nachfolgende Abbildung zeigt die Zahl der Stipendien nach Kontinenten und Forschungsbereichen (GSW: Geistes- und Sozialwissenschaften, MINT: Mathematik, Natur- und Ingenieurwissenschaften, LS: Life Sciences). In Nordamerika und Ozeanien ist die Zahl der Stipendien in den Life Sciences höher, während in den übrigen Kontinenten die Sozial- und Geisteswissenschaften am häufigsten vertreten sind. Besonders niedrig ist die Zahl der Stipendien in Lateinamerika und Afrika. Bei diesen Zahlen wurde Mexiko zu Lateinamerika gezählt.
...
df %>% 
  ...
  count(research_area) %>%
  mutate(max = max(n),
         research_area = case_when(research_area == "SSH" ~ "GSW",
                                   research_area == "MINT" ~ "MINT",
                                   research_area == "LS" ~ "LW",
                                   TRUE ~ research_area),
         research_area = factor(research_area, 
                                levels = c("GSW", "MINT", "LW")),
         Continent = case_when(Continent == "Africa" ~ "Afrika",
...
counts_plot <- counts %>%   
  ...
  mutate(research_area = case_when(research_area == "SSH" ~ "GSW",
                                   research_area == "MINT" ~ "MINT",
                                   research_area == "LS" ~ "LW",
                                   TRUE ~ research_area),
         research_area = factor(research_area,
                                levels = c("GSW", "MINT", "LW", "Total")),
         Continent = case_when(Continent == "North America" ~ "Nordamerika",
                               Continent == "Europe" ~ "Europa",
                               Continent == "Asia" ~ "Asien",
                               Continent == "Oceania" ~ "Ozeanien",
                               TRUE ~ Continent),
         Continent = factor(Continent,
                            levels = c("Nordamerika", 
                                       "Europa", 
                                       "Ozeanien", 
                                       "Asien"))) %>% 
  group_by(Continent) %>% 
  ...
  geom_point_interactive(aes(tooltip = paste0("Länder: ", Continent, "<br>",
                                              "Wissenschaftsgebiet: ", research_area, "<br>",
                                              "Anteil: ", paste0(round(pctg * 100), " %"), "<br>",
                                              "Anzahl: ", n, "<br>",
                                              "Jahr: ", CallDecisionYear, "<br>"),
...
Die folgenden Tabellen zeigen, welcher Betrag durchschnittlich nach Kontinent pro Jahr für Postdoc.Mobility-Stipendien vergeben wurde, sowie den Gesamtbetrag, der seit 2005 für die einzelnen Forschungsbereiche und Kontinente vergeben wurde. Wichtig ist der Hinweis, dass die Höhe der Stipendien teilweise von den Lebenshaltungskosten im Zielland abhängt.


<div class="plot-box">
<div class="plot-title">Durchschnittlicher Betrag pro Jahr (in Schweizer Franken) für Postdoc.Mobility-Stipendien, nach Kontinent, von 2005–2021</div>

```{r}
average <- df %>% 
  group_by(Continent) %>% 
  mutate(Continent = case_when(Continent == "North America" ~ "Nordamerika",
                               Continent == "Europe" ~ "Europa",
                               Continent == "Asia" ~ "Asien",
                               Continent == "Oceania" ~ "Ozeanien",
                               Continent == "Latin America" ~ "Lateinamerika",
...
amounts <- df %>% 
  ...
  mutate(Continent = case_when(Continent == "North America" ~ "Nordamerika",
                               ...
                               Continent == "Asia" ~ "Asien",
                               Continent == "Oceania" ~ "Ozeanien",
                               Continent == "Latin America" ~ "Lateinamerica",
                               TRUE ~ Continent),
         research_area = case_when(research_area == "SSH" ~ "GSW",
                                   research_area == "MINT" ~ "MINT",
                                   research_area == "LS" ~ "LW",
...
amounts %>% 
  kable(linesep = "",
        booktabs = TRUE,
        col.names = c("Kontinent", "GSW", "MINT", "LW", ""),
...
Daten, Text und der Code zu dieser Data Story sind auf <a href="https://github.com/snsf-data/fellowships_worldwide" target="_blank">Github</a> verfügbar und auf <a href="https://doi.org/10.46446/datastory.where-do-our-postdoc-mobility-grantees-go" target="_blank">Zenodo</a> archiviert. DOI: 10.46446/datastory.where-do-our-postdoc-mobility-grantees-go
