...
country_name_map <- c("États-Unis", "Royaume-Uni", "Allemagne", "France",
...
df <- df %>% 
  mutate(Country = case_when(Country == "United States" ~ "États-Unis",
...
plot_trend_lines <- function(df, trendy_ctries) {
  ...
  gg <- df_here %>% 
    ...
    ggplot(aes(x = CallDecisionYear, y = pctg, colour = Country,
           tooltip = paste0("Pays : ", Country, "<br>",
...
counts_plot <- counts %>%   
  ...
  mutate(research_area = case_when(research_area == "SSH" ~ "SHS",
                                   research_area == "MINT" ~ "MINT",
                                   research_area == "LS" ~ "SV",
                                   TRUE ~ research_area),
         research_area = factor(research_area,
                                levels = c("SHS", "MINT", "SV", "Totale")),
         Continent = case_when(Continent == "North America" ~ "Amérique du Nord",
                               Continent == "Europe" ~ "Europe",
                               Continent == "Asia" ~ "Asie",
                               Continent == "Oceania" ~ "Océani",
                               TRUE ~ Continent),
         Continent = factor(Continent,
                            levels = c("Amérique du Nord", 
                                       "Europe", 
                                       "Océani", 
                                       "Asie"))) %>% 
  group_by(Continent) %>% 
...
Les tableaux suivants montrent le montant moyen accordé par an aux bourses Postdoc.Mobility pour chaque continent, et le montant total accordé par domaine de recherche pour chaque continent, depuis 2005. Il est important de mentionner que le montant accordé aux bénéficiaires est partiellement défini par le coût de la vie dans le pays de destination.


<div class="plot-box">
<div class="plot-title">Montant moyen des financements accordés par an (en francs suisses) pour les bourses Postdoc.Mobility pour chaque continent de 2005 à 2021</div>

```{r}
average <- df %>% 
  group_by(Continent) %>% 
  mutate(Continent = case_when(Continent == "North America" ~ "Amérique du Nord",
                               Continent == "Europe" ~ "Europe",
                               Continent == "Asia" ~ "Asie",
                               Continent == "Oceania" ~ "Océani",
                               Continent == "Latin America" ~ "Amérique latine",
...
Les données, le texte et le code associés à ce récit de données sont disponibles sur <a href="https://github.com/snsf-data/fellowships_worldwide" target="_blank">Github</a> et archivés sur <a href="https://doi.org/10.46446/datastory.where-do-our-postdoc-mobility-grantees-go" target="_blank">Zenodo</a> DOI : 10.46446/datastory.where-do-our-postdoc-mobility-grantees-go
